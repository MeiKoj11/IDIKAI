...
function buildQuestionCard(session, index) {
  ...
  const promptLang = q.direction === "tlToEn" ? "es" : "en";
  const promptSentence = promptLang === "es" ? q.targetSentence : q.englishSentence;
  ...
  promptLabel.textContent = promptLang === "es" ? "Translate to English:" : "Translate to Spanish:";
...
async function checkSentenceTestAnswer(session, index, refs) {
  ...
  const q = session.queue[index];
  const { direction, englishSentence, targetSentence, verbFormEnglish, verbFormTarget } = q;
